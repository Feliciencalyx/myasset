...
const familyMembers = [
  { 
    ...
    role: 'PATERFAMILIAS / SYSTEM ADMIN', 
    access: 'Full Access', 
    email: 'admin@myasset-registry.com',
    status: 'ACTIVE',
    img: 'https://picsum.photos/seed/manager/200'
  },
  { 
    name: 'Legacy Trustee', 
    role: 'MATRIARCH / CO-TRUSTEE', 
    access: 'Full Access', 
    email: 'trustee@myasset-registry.com',
    status: 'ACTIVE',
    img: 'https://picsum.photos/seed/trustee/200'
  },
  { 
    name: 'Direct Heiress', 
    role: 'BENEFICIARY / HEIR', 
    access: 'Limited View', 
    email: 'heir@myasset-registry.com',
    status: 'ACTIVE',
    img: 'https://picsum.photos/seed/heir/200'
  },
];
  ...
  return (
    <div className="space-y-12">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-5xl font-black text-primary tracking-tighter font-headline">{t('family')}</h2>
          <p className="text-sm text-outline mt-2 font-medium">Managing generational roles and governance for the estate registry.</p>
        </div>
        {isAdmin && (
          <button className="bg-primary text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-3 hover:opacity-90 transition-all shadow-xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { label: 'Governance Level', value: 'Level 4: Multi-Generational', sub: 'Complex Trust Structure', icon: ShieldCheck },
          { label: 'Active Access Tokens', value: '12 / 15', sub: '3 Pending Revocation', icon: Key },
          { label: 'Security Status', value: 'Secure', sub: 'Last Audit: 4h ago', icon: ShieldAlert },
        ].map((item, i) => (
      ...
      <section className="space-y-8">
        <div className="flex justify-between items-center px-2">
          ...
          <p className="text-[10px] font-black text-outline uppercase tracking-widest">{t('totalMembers')}: {familyMembers.length}</p>
        </div>
            ...
            >
              <div className="flex items-center gap-6">
                <div className="relative">
                  <img src={member.img} alt={member.name} className="w-20 h-20 rounded-2xl object-cover grayscale" referrerPolicy="no-referrer" />
                ...
                <div>
                  ...
                  <div className="flex gap-4 mt-4 text-[10px] font-bold text-outline uppercase tracking-tighter">
                    ...
                    <span className="flex items-center gap-1.5"><ShieldCheck size={12} className="text-primary" /> {member.access}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                <div className="text-right flex flex-col items-end">
                  ...
                  <p className="text-[10px] text-outline mt-2 font-bold flex items-center gap-1 hover:text-primary transition-colors">
                    Access Ledger <ArrowUpRight size={12} />
                  </p>
                </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="bg-primary p-12 rounded-[3rem] text-white space-y-8 relative overflow-hidden">
          ...
          <div className="pt-8 flex gap-4 relative z-10">
            ...
            <button className="text-white border border-white/20 px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Review Roadmap</button>
          </div>
        </div>
        ...
        <div className="bg-surface-container-low p-10 rounded-[3rem] border border-outline-variant/10 space-y-8">
          ...
          <div className="space-y-6">
            {[
              { label: 'Direct Beneficiaries', p: 75, color: 'primary' },
              { label: 'Philanthropic Escrow', p: 15, color: 'tertiary' },
              { label: 'System Maintenance fund', p: 10, color: 'outline' },
            ].map((stat, i) => (
